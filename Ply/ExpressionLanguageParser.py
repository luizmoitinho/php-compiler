...
import * from ExpressionLanguageLex 
...
def p_statement(p):
  ...
  statement : LKEY inner_statement* RKEY
      ...
      | GLOBAL global_var LPAREN COLON global_var RPAREN* COLON
      | expr COLON
...
def p_expr_without_variable(p):
    ...
    expr_without_variable: LIST LPAREN assignment_list_element (COLON assignment_list_element)* RPAREN ASSIGN expr
      ...
      | internal_functions
      | LPAREN INT RPAREN expr
      | LPAREN DOUBLE RPAREN expr
      | LPAREN FLOAT RPAREN expr
      | LPAREN REAL RPAREN expr
      | LPAREN STRING RPAREN expr
      | LPAREN ARRAY RPAREN expr
      | LPAREN OBJECT RPAREN expr
      | LPAREN BOOL RPAREN expr
      | LPAREN BOOLEAN RPAREN expr
...
def p_simple_indirect_reference(p):
  '''
  simple_indirect_reference: (DOLAR)* 
  '''
...
def p_array_pair_list(p):
  '''
  array_pair_list : array_pair (COLON array_pair)* COLON?
  '''
